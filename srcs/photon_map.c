...
/*   Updated: 2023/05/09 16:17:54 by mstockli         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_intersect	intersect_spheres2(t_minirt minirt, t_ray ray, \
	t_intersect intersect)
{
	t_minirt		tmp;
...
void	set_photon_map(t_minirt *minirt, int i, int j)
{
	...
	while (i < 500)
	{
		...
		while (j < 500)
		{
			...
			ray.direct = make_vect(vp_pt.x - minirt->clights->origin.x, \
				vp_pt.y - minirt->clights->origin.y, vp_pt.z - \
				minirt->clights->origin.z);
