...
double get_dist_cylinder(double discriminant, double b, t_cylinder *cylinder, t_ray ray, double a)
{
	...
	height = vect_dot(diff, cylinder->normal);
...
static double	ray_cylinder_distance(t_cylinder *cylinder, t_ray ray)
{
	...
	a = vect_dot(ray.direct, ray.direct) - pow(vect_dot(ray.direct, cylinder->normal), 2);
	b = 2.0 * (vect_dot(ray.direct, oc) - vect_dot(ray.direct, cylinder->normal) * vect_dot(oc, cylinder->normal));
	c = vect_dot(oc, oc) - pow(vect_dot(oc, cylinder->normal), 2) - pow(cylinder->diameter/2.0, 2);
...
t_vect	get_cylinder_norm(t_intersect intersection, t_cylinder *cylinder)
{
	...
	t_vect 	proj = vect_scale(cylinder->normal, vect_dot(diff, cylinder->normal));
